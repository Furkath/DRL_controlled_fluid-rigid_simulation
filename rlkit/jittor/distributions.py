...
try:
    from jittor.distributions import Normal,Distribution
except ImportError:
    # print("You should use a PyTorch version that has jittor.distributions.")
    ...
    class Normal(Distribution):
        ...
        Example::
            ...
            >>> m = Normal(jittor.Tensor([0.0]), jittor.Tensor([1.0]))
            >>> m.sample()  # normally distributed with mean=0 and stddev=1
             0.1046
            [jittor.FloatTensor of size 1]
        ...
        def sample(self):
            return jittor.normal(self.mean, self.std)

        def sample_n(self, n):
            # cleanly expand float or Tensor or Variable parameters
            def expand(v):
                if isinstance(v, Number):
                    return jittor.Var([v]).expand(n, 1)
                else:
                    return v.expand(n, *v.size())
            return jittor.normal(expand(self.mean), expand(self.std))
...
class TanhNormal(Distribution):
    ...
    def sample_n(self, n, return_pre_tanh_value=False):
        ...
        if return_pre_tanh_value:
            return jittor.tanh(z), z
        else:
            return jittor.tanh(z)
    ...
    def log_prob(self, value, pre_tanh_value=None):
        ...
        if pre_tanh_value is None:
            pre_tanh_value = jittor.log(
                (1+value) / (1-value)
            ) / 2
        return self.normal.log_prob(pre_tanh_value) - jittor.log(
