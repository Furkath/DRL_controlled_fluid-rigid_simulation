...
class JittorNormalizer(Normalizer):
    """
    Update with np array, but de/normalize jittor vars.
    """
    def normalize(self, v, clip_range=None):
        ...
        if v.dim() == 2:
            ...
            std = std.unsqueeze(0)
        return jittor.clamp((v - mean) / std, -clip_range, clip_range)
...
class JittorFixedNormalizer(FixedNormalizer):
    def normalize(self, v, clip_range=None):
        if clip_range is None:
            clip_range = self.default_clip_range
        mean = ptu.from_numpy(self.mean)
