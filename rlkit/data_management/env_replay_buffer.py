...
class MultiTaskReplayBuffer(object):
    ...
    def add_enc_path(self, task, path,length):
        self.task_buffers[task].add_path(path,length)

    def add_paths(self, task, paths):
        for path in paths:
            self.task_buffers[task].add_path(path)
    def add_enc_paths(self, task, paths,length):
        for path in paths:
            self.task_buffers[task].add_enc_path(path,length)
