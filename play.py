    ...
    def penshui(self,pens:ti.f32):
        ...
        for p in self.x:
            if ( p >= self.penshuiID[None]  and p < (self.penshuiID[None] + self.flux) ) or ( (p+self.n_particles) >= self.penshuiID[None]  and (p+self.n_particles) < (self.penshuiID[None] + self.flux)  ):
                ...
                self.v[p]=  (self.outr_up[None]-self.outr_down[None])*1000000*spen
...
if __name__=="__main__":
    ...
    reslo=512#256
    gui = ti.GUI("Taichi MLS-MPM-Tube-Ball-Liquid", res=reslo, background_color=0x112F41)
    ...
    for frame in range(20000):
        ...
        for s in range(int(2e-3 // myMPM.dt)):
            myMPM.penshui(av)
            myMPM.solve_tube(ax, 0, aw)#av, aw)
            myMPM.substep()
            myMPM.solve_ball()
        #if myMPM.x.max > 
...
#        targett = np.array([[0.5, 0.85]])
#        gui.circles(targett,radius=5,color=0xff0000)
        gui.circles(myMPM.tube_x.to_numpy(), radius=1.5,color=0x845538)
        gui.circles(myMPM.ball_x.to_numpy(), radius=1.5,color=0xffb2ff)
        gui.show()
#        print(myMPM.get_reward())
#        gui.show(str(frame)+'.png')
###        imgname = f"{frame:0>{6}}"  
        #print(imgname)
###        gui.show('human/'+imgname+'.png')
